...
var defaultOptions = {
  ...
  defaultUrl: {
    value: '../files/'+pageGlobalData.name,
    kind: OptionKind.VIEWER
  },
  defaultZoomValue: {
    value: '',
    ...
    value: function render(viewport) {
      ...
      this.pdfPage.getAnnotations({ intent: intent }).then(function (annotations) {
        ...
        if (_this.div) {
          ...
          support to open audio files          
          */

          try {
            const pageNumber = parameters.page.pageIndex + 1;
            const audios = pageGlobalData.audios[pageNumber];
            ...
            if (audios)
            {
              for (let audio of audios)
              {
                ...
                audioDiv.style.top = audio.top + '%';
                audioDiv.style.left = audio.left + '%';
                audioDiv.style.width = audio.width + '%';
                audioDiv.style.height = audio.height + '%';
                audioDiv.style.zIndex = 16777271;    
                audioDiv.setAttribute('data-id', audio.id);
                audioDiv.addEventListener('click', function(e){
                  const obj = e.target;
                  const audioId = obj.dataset['id'];
                  const pdfFileNameWithoutExtension = pageGlobalData.name.split('.')[0];
                  const audioLocation = '../pdf-data/'+ pdfFileNameWithoutExtension+'/'+audioId+'.mp3';
                  // play sound
                  // console.log(audioLocation);
                  pdfSoundAudio.src = audioLocation;
                  pdfSoundAudio.play();                  
                });

                _this.div.appendChild(audioDiv);
